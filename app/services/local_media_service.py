        ...
        try:
            ...
            for track in media_info.tracks:
                if track.track_type == 'Video':
                    # pymediainfo returns duration in milliseconds; convert to
                    # seconds to match the format Jellyfin uses.
                    raw_ms = track.duration
                    duration_s = int(raw_ms / 1000) if raw_ms else None
                    info.update({
                        'duration': duration_s,
                        'width': track.width,
                        'height': track.height,
    ...
    def to_media_items(self, local_media_items: List[LocalMediaItem]) -> List[MediaItem]:
        ...
        for local_item in local_media_items:
            ...
            media_item = MediaItem(
                ...
                file_size=local_item.file_size if local_item.file_size else None
            )
            media_items.append(media_item)
