...
class JellyfinService:
    ...
    def authenticate(self, server_url: str, api_key: str, username: Optional[str] = None) -> bool:
        ...
        # Do not log API keys even partially in production builds
    ...
    def get_streaming_url(self, media_id: str) -> Optional[str]:
        ...
        try:
            ...
            # Build stream URL using string concatenation - urljoin() drops the
            # server's base path when given an absolute path argument, which breaks
            # servers hosted at a sub-path (e.g. http://host/jellyfin).
            base = self.server_url.rstrip('/')
            stream_url = (
                f"{base}/Videos/{media_id}/stream"
                f"?api_key={self.api_key}&Static=true&UserId={self.user_id}"
            )
    ...
    def _convert_jellyfin_item_to_media_item(self, item: Dict[str, Any]) -> Optional[MediaItem]:
        ...
        try:
            ...
            # Extract file size from MediaSources (first source's Size field)
            file_size = None
            media_sources = item.get('MediaSources', [])
            if media_sources and isinstance(media_sources, list):
                size_val = media_sources[0].get('Size')
                if size_val and isinstance(size_val, (int, float)) and size_val > 0:
