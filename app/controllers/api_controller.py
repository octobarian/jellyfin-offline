...
"""
from flask import Blueprint, jsonify, request, current_app, send_from_directory, send_file, Response
import logging
import os
import shutil
            ...
            try:
                for item in media_items:
                    ...
                    if hasattr(item, 'cached_thumbnail_path') and item.cached_thumbnail_path:
                        poster_url = f"/api/poster/{item.id}"
...
def get_media_details(media_id: str):
    ...
    try:
        ...
        if hasattr(media_item, 'cached_thumbnail_path') and media_item.cached_thumbnail_path:
            poster_url = f"/api/poster/{media_item.id}"
...
def play_local_media(media_id: str):
    ...
    try:
        ...
        # First attempt: go through the unified media list (normal path).
        success = media_manager.play_local_media(media_id, fullscreen=fullscreen)

        # Fallback for local_ IDs: the progressive loader Phase 1 shows items
        # with local_<hash> IDs before the unified list merges them with their
        # Jellyfin counterparts (which assigns a different ID).  Look the file
        # up directly from the local service instead of the unified list.
        if not success and media_id.startswith('local_'):
            local_items = media_manager.local_service.get_local_media(validate_files=False)
            for local_item in local_items:
                item_id = f"local_{local_item.file_hash or abs(hash(local_item.file_path))}"
                if item_id == media_id:
                    logger.info(f"Fallback local lookup succeeded for {media_id}: {local_item.file_path}")
                    success = media_manager.vlc_controller.play_local_file(
                        local_item.file_path,
                        fullscreen,
                        title=local_item.title,
                    )
                    break

        # Resolve title for the frontend modal
        play_title = ''
        try:
            item = media_manager.get_media_details(media_id)
            if item:
                play_title = item.title or ''
            elif media_id.startswith('local_'):
                for li in media_manager.local_service.get_local_media(validate_files=False):
                    if f"local_{li.file_hash or abs(hash(li.file_path))}" == media_id:
                        play_title = li.title or ''
                        break
        except Exception:
            pass
...
@api_bp.route('/disk-space')
def get_disk_space():
    """Return free/total disk space for the configured download directory."""
    try:
        config = current_app.config.get('MEDIA_CONFIG')
        path = config.download_directory if config else '/'
        # Fall back to / if the configured path doesn't exist yet
...
def get_local_media_list():
    ...
    try:
        ...
        for item in media_items:
            ...
            # Normalise duration to seconds.  Legacy DB rows written before the
            # pymediainfo ms→s fix contain milliseconds; anything >86400 (24 h)
            # is certainly ms and needs dividing.
            raw_dur = item.duration
            if raw_dur and raw_dur > 86400:
                raw_dur = int(raw_dur / 1000)
...
def get_fast_status():
    ...
    try:
        ...
        def check_internet_connectivity():
            """Fast internet connectivity check - socket only, no global timeout side-effects."""
            internet_start = time.time()
            # TCP connect to Google DNS (port 53) - fast, reliable, no HTTP overhead
            try:
                ...
                return result == 0, duration, 'socket'
            except Exception:
                pass
            duration = time.time() - internet_start
        ...
        def check_jellyfin_lightweight():
            ...
            Fast path: if the media_manager has a cached unified list it just populated
            (meaning it successfully talked to Jellyfin), trust that result for up to
            60 seconds before doing a network ping.  This avoids a slow ping on every
            30-second poll while also catching genuine outages.
            """
            ...
            # Fast path: trust remote cache freshness (5-min TTL - same window
            # the media manager itself uses to decide whether to re-fetch from Jellyfin).
            # _remote_cache_timestamp is set each time get_remote_media_list() succeeds,
            # so it's a reliable signal that Jellyfin was reachable recently.
            try:
                cache_ttl = getattr(media_manager, '_cache_ttl', 300)
                remote_ts = getattr(media_manager, '_remote_cache_timestamp', 0)
                if remote_ts and (time.time() - remote_ts) < cache_ttl:
                    return True, time.time() - jellyfin_start
            except Exception:
                pass

            # Slow path: ping /System/Info/Public (no auth required).
            # Use string concatenation - urljoin drops base paths when the second
            # arg is an absolute path (e.g. urljoin('http://host/jellyfin', '/foo')
            # → 'http://host/foo'), which breaks servers hosted at a sub-path.
            try:
                base = config.jellyfin_server_url.rstrip('/')
                public_url = base + '/System/Info/Public'
                response = requests.get(public_url, timeout=4)
                connected = response.status_code in (200, 401)  # 401 = server up, just needs auth
                return connected, time.time() - jellyfin_start
            except Exception:
                return False, time.time() - jellyfin_start
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            if status_data['services']['internet']['connected']:
                try:
                    jellyfin_connected, jellyfin_duration = jellyfin_future.result(timeout=5)
...
@api_bp.route('/poster/<media_id>')
def serve_poster(media_id: str):
    """Serve the poster image for a media item by ID.

    For local items (id starts with 'local_') we scan the local service directly
    so this endpoint works with zero Jellyfin network calls - safe for offline mode.
    For remote-only items we redirect to the Jellyfin thumbnail URL.
    """
    try:
        media_manager = current_app.config.get('MEDIA_MANAGER')
        if not media_manager:
        ...
        # ── Fast path: local items don't need Jellyfin ───────────────────────
        if media_id.startswith('local_'):
            local_service = media_manager.local_service
            local_items = local_service.get_local_media(validate_files=False)
            for local_item in local_items:
                item_id = f"local_{local_item.file_hash or abs(hash(local_item.file_path))}"
                if item_id == media_id:
                    poster_path = local_service._find_local_poster(local_item.file_path)
                    if poster_path and os.path.isfile(poster_path):
                        return send_from_directory(
                            os.path.dirname(os.path.abspath(poster_path)),
                            os.path.basename(poster_path)
                        )
                    break  # found the item but no poster on disk
            return jsonify({'error': 'No local poster found'}), 404

        # ── Slow path: remote / merged items (online only) ───────────────────
        # Use cached unified list; do NOT force refresh to avoid Jellyfin hit.
        with media_manager._cache_lock:
            cached = media_manager._unified_media_cache
        if cached:
            for item in cached:
                if item.id == media_id:
                    if getattr(item, 'cached_thumbnail_path', None) and os.path.isfile(item.cached_thumbnail_path):
                        return send_from_directory(
                            os.path.dirname(os.path.abspath(item.cached_thumbnail_path)),
                            os.path.basename(item.cached_thumbnail_path)
                        )
        ...
        logger.error(f"Error serving poster for {media_id}: {e}")
        return jsonify({'error': 'Failed to serve poster'}), 500




@api_bp.route('/media/delete/<media_id>', methods=['DELETE'])
def delete_local_media(media_id: str):
    """Delete the local file for a media item and remove it from the local DB.

    Only the local copy is deleted. Remote availability (Jellyfin) is unaffected.
    The poster file alongside the media file is also removed if present.
    """
    try:
        media_manager = current_app.config.get('MEDIA_MANAGER')
        if not media_manager:
        ...
        # Resolve local file path without triggering a Jellyfin call
        local_path = None
        local_items = local_service.get_local_media(validate_files=False)
        for local_item in local_items:
            item_id = f"local_{local_item.file_hash or abs(hash(local_item.file_path))}"
        ...
        # Also check unified cache for merged items (availability = BOTH)
        if not local_path:
            with media_manager._cache_lock:
                cached = media_manager._unified_media_cache or []
            for item in cached:
                if item.id == media_id and getattr(item, 'local_path', None):
                    local_path = item.local_path
                    break

        if not local_path:
            return jsonify({'error': 'No local file found for this item'}), 404

        if not os.path.isfile(local_path):
            return jsonify({'error': 'Local file does not exist on disk'}), 404

        # Check write permission before attempting deletion
        if not os.access(local_path, os.W_OK):
            # Try to fix permissions (works when Flask runs as root or file owner)
            try:
                import stat
                os.chmod(local_path, stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IROTH)
            except OSError:
                return jsonify({
                    'error': f'Permission denied: cannot delete {os.path.basename(local_path)}. '
                             f'The file may be owned by root. '
                             f'Run: sudo chmod a+w "{local_path}"'
                }), 403
        ...
        logger.info(f"Deleted local media file: {local_path}")

        # Delete the poster file if present (same directory, same stem)
        media_dir = os.path.dirname(local_path)
        for ext in ('.jpg', '.jpeg', '.png', '.webp'):
            for suffix in ('-poster', '.poster', '_poster'):
                candidate = os.path.join(media_dir, f"{title}{suffix}{ext}")
                if os.path.isfile(candidate):
                    os.remove(candidate)
                    deleted_files.append(candidate)
                    logger.info(f"Deleted poster file: {candidate}")

        # Remove from local DB and clear caches so the next media list is fresh
        local_service.scan_media_directories([media_dir])

        with media_manager._cache_lock:
            media_manager._unified_media_cache = None
            media_manager._cache_timestamp = 0
            media_manager._local_media_cache = []
            media_manager._local_cache_timestamp = 0
        media_manager._comparison_cache = None

        return jsonify({
            'success': True,
            'message': f'Deleted local copy of "{title}"',
            'deleted_files': deleted_files,
            'media_id': media_id
        })

    except Exception as e:
        logger.error(f"Error deleting local media {media_id}: {e}")
        return jsonify({'error': 'Failed to delete media file', 'message': str(e)}), 500
