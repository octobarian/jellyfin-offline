...
def test_vlc_installation():
    ...
    try:
        ...
        # Prefer the existing controller from the media manager so we test the
        # same VLC instance the app actually uses.  Fall back to a fresh one
        # (with optional custom path) when the media manager is unavailable.
        media_manager = current_app.config.get('MEDIA_MANAGER')
        if media_manager and not vlc_path:
            test_controller = media_manager.vlc_controller
        else:
        ...
        if is_installed:
            ...
            try:
                version_info = test_controller.get_vlc_version()
                return jsonify({
                    'success': True,
